/*
 * UNIX Directory Service POSIX Group Account Delete
...
int main(int argc, char *argv[])
{
    ...
    uxds_data_t mdata;
    char *bin = argv[0];
    sflag = parse_args(argc, argv, GROUP, DEL, 3, &auth, &mdata, bin);
    ...
    /* delete operation  */
    if (uxds_acct_del(GROUP, mdata, ld) != 0) {
	fprintf(stderr, "uxds_acct_del GROUP DELETE failed.\n");
	exit(EXIT_FAILURE);
    }
