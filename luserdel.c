/*
 * UNIX Directory Service POSIX User Account Delete 
...
int main(int argc, char *argv[])
{
    ...
    char *bin = argv[0];
    sflag = parse_argvs(argc, argv, USER, DEL, 4, &auth, &mdata, bin);
    ...
    /* delete operation */
    if (uxds_acct_del(USER, mdata, ld) != 0) {
	fprintf(stderr, "uxds_acct_del USER DELETE failed.\n");
	exit(EXIT_FAILURE);
    }
