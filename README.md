...
This project is in beta and this functionality is avaliable:
...
This is also used when authenticating with an x509 password via PK-INIT
to obtain a kerberos ticket (-K) as well as with using a keytab (-T).
...
Also, the gidNumber attribute will be changed to reflect the new group, 
as well as the GECOS field, which will reflect the changed group name.
